UnmarkedObjectFolder := C:/projects/UnivCntrl/UniversalController/Logical/HotWire/_MotorDriver
MarkedObjectFolder := C:/projects/UnivCntrl/UniversalController/Logical/HotWire/_MotorDriver

$(AS_CPU_PATH)/_MotorDriv.br: \
	$(AS_PROJECT_CPU_PATH)/Cpu.per \
	$(AS_CPU_PATH)/_MotorDriv/_MotorDriv.ox
	@"$(AS_BIN_PATH)/BR.AS.TaskBuilder.exe" "$(AS_CPU_PATH)/_MotorDriv/_MotorDriv.ox" -o "$(AS_CPU_PATH)/_MotorDriv.br" -v V1.00.0 -f "$(AS_CPU_PATH)/NT.ofs" -offsetLT "$(AS_BINARIES_PATH)/$(AS_CONFIGURATION)/$(AS_PLC)/LT.ofs" -T SG4  -M IA32  -B E4.26 -extMembers -extNames -extConstants -r Cyclic2 -p 11 -s "HotWire._MotorDriver" -L "Acp10man: V3.15.1, Acp10par: V3.15.1, AsARCfg: V*, AsBrMath: V*, AsIecCon: V*, AsIOTime: V*, AsRfbExt: V*, asstring: V*, AsTCP: V*, AsTime: V*, AsUDP: V*, AsXml: V*, bglibas: V*, brsystem: V*, DataObj: V*, dvframe: V*, Ethernet: V*, FileIO: V*, Library: V*, NcGlobal: V3.15.1, Operator: V*, Runtime: V*, sprintf2: V*, standard: V*, sys_lib: V*, Visapi: V*" -P "$(AS_PROJECT_PATH)" -secret "$(AS_PROJECT_PATH)_br.as.taskbuilder.exe"

$(AS_CPU_PATH)/_MotorDriv/_MotorDriv.ox: \
	$(AS_CPU_PATH)/_MotorDriv/a.out
	@"$(AS_BIN_PATH)/BR.AS.Backend.exe" "$(AS_CPU_PATH)/_MotorDriv/a.out" -o "$(AS_CPU_PATH)/_MotorDriv/_MotorDriv.ox" -T SG4 -r Cyclic2   -G V4.1.2  -secret "$(AS_PROJECT_PATH)_br.as.backend.exe"

$(AS_CPU_PATH)/_MotorDriv/a.out: \
	$(AS_CPU_PATH)/_MotorDriv/MotorDriverCyclic.c.o \
...
$(AS_CPU_PATH)/_MotorDriv/MotorDriverCyclic.c.o: \
	...
	@"$(AS_BIN_PATH)/BR.AS.CCompiler.exe" "$(AS_PROJECT_PATH)/Logical/HotWire/_MotorDriver/MotorDriverCyclic.c" -o "$(AS_CPU_PATH)/_MotorDriv/MotorDriverCyclic.c.o"  -T SG4  -M IA32  -B E4.26 -G V4.1.2  -s "HotWire._MotorDriver" -t "$(AS_TEMP_PATH)" -I "$(AS_PROJECT_PATH)/Logical/HotWire/_MotorDriver" "$(AS_TEMP_PATH)/Includes/HotWire/_MotorDriver"  "$(AS_TEMP_PATH)/Includes/HotWire" "$(AS_TEMP_PATH)/Includes" -trigraphs -fno-asm -D _DEFAULT_INCLUDES -D _SG4 -fPIC -O0 -g -Wall -include "$(AS_CPU_PATH)/Libraries.h" -x c -P "$(AS_PROJECT_PATH)" -secret "$(AS_PROJECT_PATH)_br.as.ccompiler.exe"
